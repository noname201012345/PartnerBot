   ...
   async def runGame(ctx, game, werewolf):
       ...
       while True:
           ...
           for x in game["dead"]:
               ...
               await vilRoom.set_permissions(target=p,send_messages=False)
               await wereRoom.set_permissions(target=p,send_messages=False)
               await Voice.set_permissions(target=p,speak = False)
           timer = 150
                   ...
                   if interaction.user.id in game["NoVote"]:
                       ...
                       for y in game["alive"]:
                           if interaction.user.id in game["alive"][y]:
                               ...
                               with open("werewolf.json","w") as f:
                                   json.dump(werewolf,f)
                               await vilRoom.send(content=f"{interaction.user.id}")
